...
module.exports = function (app, { db, dbOne, genId }) {
  ...
  app.post('/api/v1/customers', async (req, res) => {
    try {
      ...
      // Вставляем только заведомо существующие поля.
      // Если реальная таблица customers требует ещё колонку без дефолта —
      // Postgres вернёт "null value in column X" и мы добавим её сюда.
      await db(`
      ...
      res.status(201).json({ id, name, phone, email, status: 'ACTIVE' });
    } catch (err) {
