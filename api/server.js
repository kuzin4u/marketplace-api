...
app.post('/api/v1/shops', async (req, res) => {
  try {
    const { seller_id, slug, brand_name, brand_tagline, brand_accent,
            brand_description, channels, category, region } = req.body;

    if (!seller_id || !slug || !brand_name) {
      return res.status(400).json({ error: 'seller_id, slug, brand_name required' });
    }
...
// Системный промпт собирается из каталога
async function buildAgentSystemPrompt() {
  ...
  const productsText = products.map(p => {
    ...
    return [
      p.title, `Пекарня: ${p.brand_name}`, `Цена: ${p.price}₽`,
      `Вес: ${p.weight_g || '?'}г`, `₽/кг: ${pricePerKg || '?'}`,
      `Рейтинг: ${p.rating} (${p.reviews_count} отзывов)`,
      `Заказов: ${p.orders_count}`, `В наличии: ${p.stock} шт`,
      `Описание: ${(p.description || '').substring(0, 120)}`
    ].join(' | ');
  }).join('\n');
...
8. Используй markdown таблицы для сравнений.
