...
// сразу нельзя — на него завязаны формы. Поэтому: доступ по токену,
// а идентификатор из адреса лишь сверяется с токеном и ни на что не влияет.
// Когда формы перейдут на /api/v1/seller/products без параметра,
// эта проверка и сам параметр убираются.
