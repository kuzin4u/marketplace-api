...
// доступ по токену, а идентификатор из адреса лишь сверяется с токеном
// и ни на что не влияет.
// Ошибка ограничения базы — не «Internal error», а понятная причина.
// Форма показывает этот текст продавцу, и «нет такой категории»
// куда полезнее, чем «внутренняя ошибка».
function fromDbError(res, err, where) {
  if (err && err.code === '23505')
    return res.status(409).json({ error: 'already_exists', detail: 'Запись с таким значением уже есть' });
  if (err && err.code === '23503')
    return res.status(422).json({ error: 'unknown_reference', detail: 'Указана несуществующая категория или магазин' });
  if (err && err.code === '23514')
    return res.status(422).json({ error: 'constraint_violation', detail: 'Значение недопустимо: ' + (err.constraint || err.message) });
  if (err && err.code === '23502')
    return res.status(422).json({ error: 'missing_required_field', detail: 'Не заполнено обязательное поле: ' + (err.column || '') });
  console.error(where || 'db', err && err.message);
  return res.status(500).json({ error: 'Internal error' });
}

function assertSelf(req, res) {
  const fromUrl = req.params.seller_id;
  if (fromUrl && fromUrl !== req.seller_id) {
    res.status(403).json({ error: 'not_your_account',
      detail: 'Идентификатор в адресе не совпадает с токеном' });
    return false;
  }
  return true;
}
...
app.get(['/api/v1/seller/products', '/api/v1/seller/:seller_id/products'], requireSeller, async (req, res) => {
  try {
...
app.post(['/api/v1/seller/products', '/api/v1/seller/:seller_id/products'], requireSeller, async (req, res) => {
  try {
    if (!assertSelf(req, res)) return;
    const { title, description, price, stock, category_id, tags, images,
            weight_g, shipping_zones, visible_catalog = true } = req.body;
    const seller_id = req.seller_id;   // из токена, не из адреса
    ...
    return fromDbError(res, err, 'POST /seller/products');
  }
});

app.patch(['/api/v1/seller/products/:sku_id', '/api/v1/seller/:seller_id/products/:sku_id'], requireSeller, async (req, res) => {
  try {
    if (!assertSelf(req, res)) return;
    ...
    // Владение проверяется в самом UPDATE: seller_id берётся из токена,
    // поэтому чужой товар не совпадёт по условию и строк не изменится.
    params.push(req.params.sku_id);
    params.push(req.seller_id);

    const result = await db(
      `UPDATE skus SET ${updates.join(', ')} WHERE id = $${idx} AND seller_id = $${idx + 1} RETURNING id`,
      params);

    // Раньше ответ был «updated: true» даже когда не обновилось ничего:
    // на чужой товар приходил успех, и это выглядело как работающая правка.
    if (result.length === 0)
      return res.status(404).json({ error: 'not_found_or_not_yours' });

    res.json({ updated: true, id: result[0].id });
  } catch (err) {
    res.status(500).json({ error: 'Internal error' });
  }
});
...
app.get(['/api/v1/seller/stats', '/api/v1/seller/:seller_id/stats'], requireSeller, async (req, res) => {
  try {
    if (!assertSelf(req, res)) return;
...
app.post('/api/v1/participants', async (req, res) => {
  try {
    ...
    return fromDbError(res, err, 'POST /participants');
  }
});
...
app.get('/api/v1/smoke-test', async (req, res) => {
  ...
  // Колонки orders.total в схеме нет и никогда не было — проверка всё это
  // время падала с ошибкой и ничего не проверяла. Верное имя — total_amount.
  await check('clearing_balance', async () => {
    const r = await dbOne(`
      SELECT COUNT(*)::int AS bad FROM (
        SELECT o.id, o.total_amount::numeric AS total,
               COALESCE(SUM(cs.amount), 0)::numeric AS split
        FROM orders o
        LEFT JOIN clearing_splits cs ON cs.order_id = o.id
        GROUP BY o.id, o.total_amount
        HAVING ABS(o.total_amount::numeric - COALESCE(SUM(cs.amount), 0)::numeric) > 0.01
